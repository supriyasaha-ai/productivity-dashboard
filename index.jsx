...
/* ─── Design tokens ──────────────────────────────────────────────────────────
   Warm & soft pastel palette. Cream base, dusty rose accents, sage greens,
   periwinkle blues. Feels like a well-organised journal, not a SaaS tool.
──────────────────────────────────────────────────────────────────────────── */
const T = {
  cream:      "#FBF8F3",
  parchment:  "#F5F0E8",
  ...
  periwinkle: "#A8B4E8",
  periwinkleDark: "#3C4FAA",
  periwinklePale: "#EEF0FB",
...
function exportCSV(apps) {
  ...
  const rows = apps.map(a => [
    a.company, a.role, a.dateApplied, a.platform, a.status, a.priority || "Medium",
    a.salaryRange, a.expectedSalary, a.contactName, a.notes
  ].map(v => `"${(v||"").replace(/"/g,'""')}"`));
...
function FieldLabel({ children }) {
  return <p style={{ margin:"0 0 5px", fontSize:11, fontWeight:600, color:T.inkLight, textTransform:"uppercase", letterSpacing:"0.07em" }}>{children}</p>;
}

function Pill({ children, bg, color, dot }) {
  return (
...
function Toggle({ label, value, onChange }) {
  return (
    <label style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:5, cursor:"pointer" }}>
      <span style={{ fontSize:11, color:T.inkLight, fontWeight:500 }}>{label}</span>
      <div onClick={() => onChange(!value)} style={{
        width:40, height:22, borderRadius:11, cursor:"pointer",
        background: value ? T.sage : T.borderMid,
        position:"relative", transition:"background .2s", flexShrink:0,
        border:`1.5px solid ${value ? T.sageDark+"44" : T.borderMid}`,
      }}>
        <span style={{
...
function StatCard({ label, value, accent, sub }) {
  return (
    <div style={{
      ...
      boxShadow:"0 1px 4px rgba(45,36,32,.04)",
    }}>
      <span style={{ fontSize:11, color:T.inkLight, fontWeight:600, textTransform:"uppercase", letterSpacing:"0.07em" }}>{label}</span>
      <span style={{ fontSize:30, fontWeight:700, color: accent || T.ink, lineHeight:1, fontFamily:"'DM Serif Display', Georgia, serif" }}>{value}</span>
      {sub && <span style={{ fontSize:11, color:T.inkLight }}>{sub}</span>}
    </div>
  );
}
...
function SectionHeader({ title, subtitle, action }) {
  return (
    <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", marginBottom:"1.25rem" }}>
      <div>
        <h2 style={{ margin:0, fontSize:18, fontWeight:700, color:T.ink, fontFamily:"'DM Serif Display', Georgia, serif", letterSpacing:"-0.02em" }}>{title}</h2>
        {subtitle && <p style={{ margin:"2px 0 0", fontSize:12, color:T.inkLight }}>{subtitle}</p>}
      </div>
      {action}
    </div>
  );
...
function Btn({ children, onClick, variant="ghost", small=false }) {
  const base = {
    ...
    fontSize: small ? 12 : 13, display:"inline-flex", alignItems:"center", gap:5,
    padding: small ? "5px 12px" : "8px 16px", transition:"all .15s", fontFamily:"inherit",
  };
  const styles = {
    ...
    periwinkle:{ ...base, background:T.periwinklePale, color:T.periwinkleDark, border:`1px solid ${T.periwinkle}77` },
  };
  return <button onClick={onClick} style={styles[variant] || styles.ghost}>{children}</button>;
}

function TextInput({ value, onChange, placeholder, full=true, type="text" }) {
  return (
    <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
      style={{
        width: full ? "100%" : undefined, boxSizing:"border-box",
        padding:"9px 12px", fontSize:13, border:`1.5px solid ${T.border}`,
        borderRadius:9, background:T.cardBg, color:T.ink, fontFamily:"inherit",
...
function SelectInput({ value, onChange, children, full=true }) {
  return (
...
function ModalShell({ title, onClose, onSave, saveLabel="Save", saveVariant="primary", children }) {
  return (
    <div style={{
      position:"fixed", inset:0, background:"rgba(45,36,32,.35)", backdropFilter:"blur(4px)",
      display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:"1rem",
    }} onClick={e => e.target===e.currentTarget && onClose()}>
      <div style={{
        background:T.cream, border:`1px solid ${T.border}`,
        borderRadius:18, padding:"1.5rem", width:"100%", maxWidth:580,
        maxHeight:"90vh", overflowY:"auto", boxShadow:"0 24px 60px rgba(45,36,32,.18)",
      }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1.5rem" }}>
          <h2 style={{ margin:0, fontSize:17, fontWeight:700, color:T.ink, fontFamily:"'DM Serif Display', Georgia, serif" }}>{title}</h2>
          <button onClick={onClose} style={{ background:"none", border:"none", cursor:"pointer", fontSize:22, color:T.inkLight, lineHeight:1, padding:"2px 6px" }}>×</button>
        </div>
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"flex-end", marginTop:"1.5rem" }}>
          ...
          <Btn onClick={onSave} variant={saveVariant}>{saveLabel}</Btn>
        </div>
      </div>
    </div>
  );
...
const emptyApp = () => ({
  ...
  emailSent:false, linkedinSent:false,
...
function JobModal({ app, onSave, onClose }) {
  ...
  return (
    <ModalShell
      title={app.company ? "Edit Application" : "✨ New Application"}
      onClose={onClose}
      onSave={() => { if (!form.company.trim()) return; onSave(form); }}
      saveLabel="Save Application"
    >
      <div style={{ gridColumn:"1/-1" }}>
        <FieldLabel>Company Name</FieldLabel>
        <TextInput value={form.company} onChange={v=>set("company",v)} placeholder="e.g. Nykaa" />
      </div>
      <div style={{ gridColumn:"1/-1" }}>
        <FieldLabel>Role / Position</FieldLabel>
        <TextInput value={form.role} onChange={v=>set("role",v)} placeholder="e.g. Product Marketing Manager" />
      </div>
      <div>
        <FieldLabel>Date Applied</FieldLabel>
        <TextInput type="date" value={form.dateApplied} onChange={v=>set("dateApplied",v)} />
      </div>
      <div>
        <FieldLabel>Platform</FieldLabel>
        <SelectInput value={form.platform} onChange={v=>set("platform",v)}>
          {PLATFORMS.map(p=><option key={p}>{p}</option>)}
        </SelectInput>
      </div>
      <div>
        <FieldLabel>Priority</FieldLabel>
        <SelectInput value={form.priority||"Medium"} onChange={v=>set("priority",v)}>
      ...
      <div>
        ...
        <TextInput value={form.contactName} onChange={v=>set("contactName",v)} placeholder="Name" />
      </div>
      <div>
        <FieldLabel>Contact Email / LinkedIn</FieldLabel>
        <TextInput value={form.contactLink} onChange={v=>set("contactLink",v)} placeholder="email or profile URL" />
      </div>
      <div>
        <FieldLabel>Salary Range (Role)</FieldLabel>
        <TextInput value={form.salaryRange} onChange={v=>set("salaryRange",v)} placeholder="e.g. ₹10–15 LPA" />
      </div>
      <div>
        <FieldLabel>My Expected Salary</FieldLabel>
        <TextInput value={form.expectedSalary} onChange={v=>set("expectedSalary",v)} placeholder="e.g. ₹12 LPA" />
      </div>
      <div style={{ display:"flex", gap:28, alignItems:"center", paddingTop:4 }}>
      ...
      <div style={{ gridColumn:"1/-1" }}>
        ...
        <textarea value={form.notes} onChange={e=>set("notes",e.target.value)}
          rows={3} placeholder="e.g. Follow up Friday, ask about team size..."
          style={{ width:"100%", boxSizing:"border-box", resize:"vertical", fontFamily:"inherit",
            fontSize:13, padding:"9px 12px", border:`1.5px solid ${T.border}`,
            borderRadius:9, background:T.cardBg, color:T.ink, outline:"none" }}
          onFocus={e=>e.target.style.borderColor=T.rose}
          onBlur={e=>e.target.style.borderColor=T.border}
        />
      </div>
...
function JobTracker() {
  ...
  useEffect(() => {
    storageGet("jt_apps").then(v => { if (v) setApps(v); setLoaded(true); });
  }, []);

  useEffect(() => {
    if (loaded) storageSet("jt_apps", apps);
  }, [apps, loaded]);
  ...
  const quickStatus = (id, status) => setApps(prev => prev.map(a => a.id===id ? {...a, status} : a));
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:"1.5rem" }}>
        ...
        <StatCard label="Need Follow-up" value={needFollowUp}
          accent={needFollowUp>0 ? T.mustardDark : undefined}
          sub={needFollowUp>0 ? "5+ days, no update" : "All good ✓"} />
        <StatCard label="Interviews" value={interviews} accent={T.lavDark} />
        <StatCard label="Offers" value={offers} accent={T.sageDark}
          sub={offers>0 ? "🎉 Congrats!" : undefined} />
      </div>
      ...
      <div style={{
        ...
        <div style={{ position:"relative", flex:"1 1 180px", minWidth:140 }}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)}
            ...
            style={{ width:"100%", boxSizing:"border-box", padding:"7px 10px 7px 32px", fontSize:13,
              border:`1.5px solid ${T.border}`, borderRadius:9, background:T.cream,
              color:T.ink, fontFamily:"inherit", outline:"none" }}
            onFocus={e=>e.target.style.borderColor=T.rose}
            onBlur={e=>e.target.style.borderColor=T.border}
          />
        </div>
        <SelectInput value={filterStatus} onChange={setFilterStatus} full={false}>
          <option value="All">All statuses</option>
          {STATUSES.map(s=><option key={s}>{s}</option>)}
        </SelectInput>
        <SelectInput value={filterPlatform} onChange={setFilterPlatform} full={false}>
          <option value="All">All platforms</option>
          {PLATFORMS.map(p=><option key={p}>{p}</option>)}
        </SelectInput>
        <SelectInput value={filterPriority} onChange={setFilterPriority} full={false}>
          <option value="All">All priorities</option>
          {PRIORITIES.map(p=><option key={p}>{p}</option>)}
        </SelectInput>
        <div style={{ flex:1 }} />
        {apps.length > 0 && (
          <Btn onClick={() => exportCSV(apps)} variant="ghost" small>⬇ CSV</Btn>
        )}
      ...
      {highPri > 0 && (
        <div style={{
          background:T.roseBg, border:`1px solid ${T.rose}88`, borderRadius:10,
          padding:"9px 14px", marginBottom:"1rem", fontSize:13, color:T.roseDark,
          display:"flex", alignItems:"center", gap:8,
        }}>
          <span style={{ fontSize:16 }}>🔥</span>
          <strong>{highPri} high-priority</strong> application{highPri>1?"s":""} need your attention
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <div style={{
          textAlign:"center", padding:"4rem 2rem", color:T.inkLight,
          background:T.cardBg, border:`1px solid ${T.border}`, borderRadius:14,
        }}>
          <div style={{ fontSize:36, marginBottom:"0.75rem" }}>📋</div>
          <div style={{ fontSize:15, fontWeight:600, color:T.inkMid, marginBottom:4 }}>
            {apps.length===0 ? "No applications yet" : "No results match your filters"}
          </div>
          <div style={{ fontSize:13 }}>
            {apps.length===0 ? "Start tracking your job hunt — add your first application!" : "Try adjusting your search or filters"}
          </div>
        </div>
      ) : (
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
              ...
              <tr style={{ background:T.parchment }}>
                {["","Company / Role","Applied","Platform","Status","Salary","Outreach","Actions"].map(h=>(
                  <th key={h} style={{ padding:"10px 12px", textAlign:"left", fontWeight:600,
                    color:T.inkLight, fontSize:10.5, textTransform:"uppercase",
            ...
            <tbody>
              {filtered.map((app,i) => {
                ...
                return (
                  <tr key={app.id} style={{
                    background: stale ? T.mustardPale+"88" : i%2===0 ? T.cardBg : T.cream,
                    borderLeft: stale ? `3px solid ${T.mustard}` : `3px solid transparent`,
                    transition:"background .15s",
                  }}>
                    {/* priority dot */}
                    <td style={{ padding:"10px 8px 10px 12px", borderBottom:`1px solid ${T.border}` }}>
                      <span title={app.priority||"Medium"} style={{
                        display:"block", width:8, height:8, borderRadius:"50%", background:pri.dot,
                      }} />
                    </td>
                    {/* company + role */}
                    <td style={{ padding:"10px 12px", borderBottom:`1px solid ${T.border}`, maxWidth:220 }}>
                      ...
                      {stale && urg && (
                        <div style={{ fontSize:10.5, color:urg.color, marginTop:3, fontWeight:600 }}>
                          ⏰ {days}d — follow up!
                        </div>
                      )}
                    </td>
                    ...
                    <td style={{ padding:"10px 12px", borderBottom:`1px solid ${T.border}` }}>
                      <select value={app.status}
                        onChange={e => quickStatus(app.id, e.target.value)}
                        style={{
                          fontSize:11, fontWeight:600, padding:"3px 8px", borderRadius:20,
                          background:sm.bg, color:sm.text, border:`0.5px solid ${sm.dot}55`,
                    ...
                    <td style={{ padding:"10px 12px", borderBottom:`1px solid ${T.border}`, whiteSpace:"nowrap" }}>
                      <div style={{ display:"flex", gap:5 }}>
                        <span style={{ fontSize:11, color: app.emailSent ? T.sageDark : T.inkLight, fontWeight: app.emailSent ? 600 : 400 }}>
                          {app.emailSent ? "✓ Email" : "○ Email"}
                        </span>
                        <span style={{ color:T.border }}>·</span>
                        <span style={{ fontSize:11, color: app.linkedinSent ? "#5A8FD4" : T.inkLight, fontWeight: app.linkedinSent ? 600 : 400 }}>
                          {app.linkedinSent ? "✓ LI" : "○ LI"}
                        </span>
                      </div>
                      {app.contactName && (
                        <div style={{ fontSize:11, color:T.inkLight, marginTop:2 }}>{app.contactName}</div>
                      )}
                    </td>
                    {/* actions */}
                    <td style={{ padding:"10px 12px", borderBottom:`1px solid ${T.border}` }}>
...
const emptyContent = () => ({
  id: Date.now() + Math.random(),
  title:"", type:"LinkedIn Post", status:"Idea",
  platform:"", dueDate:"", keywords:"", hook:"", cta:"", notes:"",
...
function ContentModal({ item, onSave, onClose }) {
  ...
  return (
    <ModalShell
      title={item.title ? "Edit Content" : "✨ New Content Piece"}
      onClose={onClose}
      onSave={() => { if (!form.title.trim()) return; onSave(form); }}
      saveLabel="Save Piece"
      saveVariant="sage"
    >
      <div style={{ gridColumn:"1/-1" }}>
        <FieldLabel>Title / Topic</FieldLabel>
        <TextInput value={form.title} onChange={v=>set("title",v)} placeholder="e.g. How I managed 550+ brands with AI" />
      </div>
      <div>
      ...
      <div>
        ...
        <TextInput value={form.platform} onChange={v=>set("platform",v)} placeholder="e.g. LinkedIn, Personal blog..." />
      </div>
      <div>
      ...
      <div style={{ gridColumn:"1/-1" }}>
        ...
        <TextInput value={form.keywords} onChange={v=>set("keywords",v)} placeholder="e.g. career, AI, ecommerce" />
      </div>
      <div style={{ gridColumn:"1/-1" }}>
        <FieldLabel>Hook / Opening Line</FieldLabel>
        <TextInput value={form.hook} onChange={v=>set("hook",v)} placeholder="What's the scroll-stopping first line?" />
      </div>
      <div style={{ gridColumn:"1/-1" }}>
        <FieldLabel>CTA / Goal</FieldLabel>
        <TextInput value={form.cta} onChange={v=>set("cta",v)} placeholder="e.g. Book a call, Subscribe, Share" />
      </div>
      <div style={{ gridColumn:"1/-1" }}>
        <FieldLabel>Notes / Outline</FieldLabel>
        <textarea value={form.notes} onChange={e=>set("notes",e.target.value)}
          rows={4} placeholder="Rough outline, references, inspiration..."
          style={{ width:"100%", boxSizing:"border-box", resize:"vertical", fontFamily:"inherit",
            fontSize:13, padding:"9px 12px", border:`1.5px solid ${T.border}`,
            borderRadius:9, background:T.cardBg, color:T.ink, outline:"none" }}
...
function ContentPlanner() {
  ...
  useEffect(() => {
    storageGet("cp_items").then(v => { if (v) setItems(v); setLoaded(true); });
  }, []);

  useEffect(() => {
    if (loaded) storageSet("cp_items", items);
  }, [items, loaded]);
  ...
  const moveStatus = (id, s) => setItems(prev => prev.map(i => i.id===id ? {...i, status:s} : i));
    ...
    <div>
      ...
      {overdueCount > 0 && (
        <div style={{
          ...
          <span>⚠️</span>
          <strong>{overdueCount} piece{overdueCount>1?"s are":" is"} past due</strong>
          — update the status or push the deadline
        </div>
      )}
      ...
      <div style={{
        ...
        <SelectInput value={filterType} onChange={setFilterType} full={false}>
        ...
        <div style={{ display:"flex", border:`1px solid ${T.border}`, borderRadius:9, overflow:"hidden" }}>
          {[["board","⊞ Board"],["list","☰ List"]].map(([v,label])=>(
            <button key={v} onClick={()=>setView(v)} style={{
              background: view===v ? T.parchment : "transparent",
              border:"none", padding:"6px 14px", cursor:"pointer", fontSize:12,
              fontWeight: view===v ? 600 : 400,
              color: view===v ? T.ink : T.inkLight,
              fontFamily:"inherit",
            }}>{label}</button>
          ))}
        </div>
      ...
      {view==="board" ? (
        <BoardView items={filtered} onEdit={setModal} onDelete={del} onMove={moveStatus} />
      ) : (
        <ContentListView items={filtered} onEdit={setModal} onDelete={del} />
      )}
    </div>
  );
}
      ...
      {CONTENT_STATUSES.map(status => {
        ...
        return (
          <div key={status} style={{
            background: T.parchment, borderRadius:12, padding:10, border:`1px solid ${T.border}`,
          }}>
            <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:10 }}>
              <span style={{ width:7, height:7, borderRadius:"50%", background:m.dot, flexShrink:0 }} />
              <span style={{ fontSize:11, fontWeight:600, color:T.inkMid }}>{status}</span>
              <span style={{
                marginLeft:"auto", fontSize:10, background:m.bg, color:m.text,
                border:`0.5px solid ${m.dot}66`, borderRadius:10, padding:"1px 7px", fontWeight:600,
              }}>{col.length}</span>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:8, maxHeight:500, overflowY:"auto" }}>
              {col.length===0 && (
                <div style={{ textAlign:"center", padding:"1.5rem 0.5rem", color:T.inkLight, fontSize:12 }}>Empty</div>
              )}
              {col.map(item => {
                ...
                return (
                  <div key={item.id} style={{
                    ...
                    borderTop: overdue ? `2px solid ${T.rose}` : `2px solid transparent`,
                  }}>
                    <div style={{ fontSize:12, fontWeight:600, color:T.ink, marginBottom:3, lineHeight:1.35 }}>
                      {TYPE_ICONS[item.type]} {item.title}
                    </div>
                    <div style={{ fontSize:10.5, color:T.inkLight, marginBottom:6 }}>{item.type}</div>
                    {item.dueDate && (
                      <div style={{ fontSize:10.5, color: overdue ? T.roseDark : T.inkLight, fontWeight: overdue ? 600 : 400 }}>
                        📅 {new Date(item.dueDate).toLocaleDateString("en-IN",{day:"numeric",month:"short"})}
                        {overdue && " — overdue!"}
                      </div>
                    )}
                    {item.keywords && (
                      <div style={{ fontSize:10, color:T.inkLight, marginTop:4, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                        🏷 {item.keywords}
                      </div>
                    )}
                    {/* quick move */}
                    <select value={status}
                      ...
                      style={{
                        marginTop:7, width:"100%", fontSize:10.5, padding:"3px 6px",
                        border:`0.5px solid ${T.border}`, borderRadius:6,
                        background:T.cream, color:T.inkMid, fontFamily:"inherit", cursor:"pointer",
                      }}>
                      {CONTENT_STATUSES.map(s=><option key={s}>{s}</option>)}
                    </select>
                    <div style={{ display:"flex", gap:5, marginTop:6 }}>
                      <button onClick={()=>onEdit({...item})} style={{
                        flex:1, background:T.parchment, border:`0.5px solid ${T.border}`,
                        borderRadius:6, padding:"3px 0", cursor:"pointer", fontSize:11,
                        color:T.inkMid, fontFamily:"inherit",
                      }}>Edit</button>
                      <button onClick={()=>onDelete(item.id)} style={{
                        background:T.roseBg, border:`0.5px solid ${T.rose}55`,
                        color:T.roseDark, borderRadius:6, padding:"3px 8px", cursor:"pointer", fontSize:11,
                        fontFamily:"inherit",
                      }}>✕</button>
                    </div>
                  </div>
...
function ContentListView({ items, onEdit, onDelete }) {
  if (items.length===0) return (
    <div style={{
      ...
      <div style={{ fontSize:15, fontWeight:600, color:T.inkMid }}>No content pieces yet</div>
      <div style={{ fontSize:13, marginTop:4 }}>Start planning your content — add your first piece!</div>
    </div>
  );
  return (
    <div style={{ border:`1px solid ${T.border}`, borderRadius:14, overflow:"hidden", boxShadow:"0 1px 4px rgba(45,36,32,.04)" }}>
        ...
        <thead>
          <tr style={{ background:T.parchment }}>
            {["Title","Type","Status","Due","Keywords","Actions"].map(h=>(
              <th key={h} style={{ padding:"10px 12px", textAlign:"left", fontWeight:600,
                color:T.inkLight, fontSize:10.5, textTransform:"uppercase", letterSpacing:"0.07em",
          ...
          {items.map((item,i) => {
            ...
            return (
              <tr key={item.id} style={{ background: i%2===0 ? T.cardBg : T.cream }}>
                <td style={{ padding:"10px 12px", borderBottom:`1px solid ${T.border}` }}>
                  <div style={{ fontWeight:600, color:T.ink }}>{TYPE_ICONS[item.type]} {item.title}</div>
                  {item.hook && <div style={{ fontSize:11, color:T.inkLight, marginTop:2 }}>{item.hook.slice(0,70)}{item.hook.length>70?"…":""}</div>}
                </td>
                <td style={{ padding:"10px 12px", borderBottom:`1px solid ${T.border}`, color:T.inkMid, whiteSpace:"nowrap" }}>{item.type}</td>
                <td style={{ padding:"10px 12px", borderBottom:`1px solid ${T.border}` }}>
                  <Pill bg={m.bg} color={m.text} dot={m.dot}>{item.status}</Pill>
                </td>
                <td style={{ padding:"10px 12px", borderBottom:`1px solid ${T.border}`, whiteSpace:"nowrap",
                  color: overdue ? T.roseDark : T.inkMid, fontWeight: overdue ? 600 : 400 }}>
                  {item.dueDate ? new Date(item.dueDate).toLocaleDateString("en-IN",{day:"numeric",month:"short"}) : "—"}
                  {overdue && " ⚠️"}
                </td>
                <td style={{ padding:"10px 12px", borderBottom:`1px solid ${T.border}`, color:T.inkLight, maxWidth:140 }}>
                  <span style={{ display:"block", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{item.keywords||"—"}</span>
                </td>
                <td style={{ padding:"10px 12px", borderBottom:`1px solid ${T.border}` }}>
                  <div style={{ display:"flex", gap:5 }}>
                    <Btn onClick={()=>onEdit({...item})} variant="ghost" small>Edit</Btn>
                    <Btn onClick={()=>onDelete(item.id)} variant="danger" small>✕</Btn>
                  </div>
                </td>
              </tr>
            );
...
export default function Dashboard() {
  ...
  return (
    <div style={{
      ...
      maxWidth:1140, margin:"0 auto", padding:"2rem 1.25rem 4rem",
    }}>
      {/* Google font */}
      <style>{`
      ...
      <div style={{ marginBottom:"2rem" }}>
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", flexWrap:"wrap", gap:"1rem" }}>
          <div>
            <h1 style={{
              margin:"0 0 4px", fontSize:28, fontWeight:700, color:T.ink,
              fontFamily:"'DM Serif Display', Georgia, serif", letterSpacing:"-0.03em",
            }}>
        ...
        {/* Tabs */}
        <div style={{
          display:"inline-flex", gap:3, marginTop:"1.25rem",
          ...
          {[
            ...
            <button key={t.id} onClick={()=>setTab(t.id)} style={{
              ...
              boxShadow: tab===t.id ? "0 1px 4px rgba(45,36,32,.07)" : "none",
              borderRadius:9, padding:"8px 22px", cursor:"pointer",
              fontSize:13, fontWeight: tab===t.id ? 600 : 500,
              color: tab===t.id ? T.ink : T.inkMid,
      ...
      {tab==="jobs" ? (
        <>
          <SectionHeader
            ...
            subtitle="Track every application, follow-up, and offer in one place"
          />
          <JobTracker />
        </>
      ) : (
        <>
          <SectionHeader
            ...
            subtitle="Plan, draft, and publish content that builds your personal brand"
          />
          <ContentPlanner />
        </>
      )}
